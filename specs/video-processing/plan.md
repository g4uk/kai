...
Each step above is its own commit: red-phase test commits and their corresponding green-phase implementation commits are separate (never squashed together), per CLAUDE.md's "tests ship in the same commit as their implementation" — read precisely, that rule means a test and the code that makes it pass land together at the *feature* level (this plan), not that red and green are one commit; each step's red commit documents the intended contract before the immediately-following green commit satisfies it.
